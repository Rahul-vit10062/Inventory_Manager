...
class _SalesEntryFormState extends State<SalesEntryForm> {
  ...
  List<Map> itemNamesAndNicknames = new List.empty();
  String disclaimerText = '';
  String stringUnderName = '';
  String tempItemId;
  bool enableAdvancedFields = false;

  List units = new List.empty();
  String selectedUnit = '';
  ...
  void _initiateTransactionData() {
    ...
    if (this.widget.swipeData != null) {
      ...
      this.units = item.units?.keys?.toList() ?? new List.empty();
  ...
  void updateItemName() {
    ...
    itemFuture.then((item) {
      if (item == null) {
        ...
        setState(() => this.units = new List.empty());
      } else {
  ...
  void _initializeItemNamesAndNicknamesMapCache() async {
    ...
    List<Map> cacheItemAndNickNames = []; //new List.empty();
